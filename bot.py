...
def subscribe_keyboard() -> InlineKeyboardMarkup:
    channel_buttons = [
        InlineKeyboardButton(text=f"{i}️⃣ Kanalga o'tish", url=f"https://t.me/{c.lstrip('@')}")
        for i, c in enumerate(config.CHANNELS, start=1)
    ]
    rows = [channel_buttons[i:i + 2] for i in range(0, len(channel_buttons), 2)]
    rows.append([_btn("✅ Tekshirish", "check_sub")])
...
def main_menu_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if user_id == config.ADMIN_ID:
        rows.append([_btn("🌐 Proxy server", "menu_proxy"), _btn("📊 Statistika", "menu_stats")])
    else:
        rows.append([_btn("🌐 Proxy server", "menu_proxy")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
